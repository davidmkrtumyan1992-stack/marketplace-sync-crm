...
export async function fetchYandexProducts(oauthToken: string, businessId: string): Promise<NormalizedProduct[]> {
  ...
  // Derive actual Business ID from discovered campaigns (warehouseId may be Campaign ID, not Business ID)
  const actualBusinessId = campaigns.find(c => c.businessId && c.businessId !== "unknown")?.businessId || cleanBusinessId;
  console.log(`[Yandex Sync] Using Business ID for offer-mappings: ${actualBusinessId} (from campaigns=${actualBusinessId !== cleanBusinessId ? "discovered" : "settings"})`);

  // Step 2: Fetch products by actual Business ID
  let allEntries: any[] = [];
  try {
    console.log(`[Yandex Sync] === STEP 2: POST /businesses/${actualBusinessId}/offer-mappings ===`);
    allEntries = await fetchYandexOffersByBusiness(BASE, headers, actualBusinessId);
    console.log(`[Yandex Sync] Business-level fetch returned ${allEntries.length} offer(s)`);
  } catch (err: any) {
    console.log(`[Yandex Sync] Step 2 failed (${err.message}) — no per-campaign fallback for offer-mappings`);
  }
