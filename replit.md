...
- getSalesData MSK timezone fix: `getSalesData` in `server/storage.ts` now uses `toLocaleDateString("en-CA", { timeZone: "Europe/Moscow" })` for all date comparisons and chart grouping instead of UTC `.toISOString().split("T")[0]`. Revenue is now computed from `order_items` SUM(price × qty) instead of `order.totalAmount`. `totalOrders` field now returns SUM of item quantities (штуки). Cancelled filter simplified: exclude only when `status=cancelled AND (ozonStatus IS NULL OR ozonStatus=cancelled)`. This fixes the root cause of the 13 vs 14 order discrepancy for Texnicol on March 20.
